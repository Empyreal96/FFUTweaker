...
@echo usage:
@echo.
@echo sign [-wscv ^| -hal ^| -msapp ^| -app ^| -app2 ^| -app3 ^| -msappx ^| -appx2 ^| -appx3 ^| -pp ^| -ppl ^| -pk ^| -ext ^| -uap1 ^| -uap2 ^| -uap3] [-ph] ^<file1^> ^<file2^> [..]  -- signs file1, file2, etc.
@echo.
...
if "%CERTIFICATEFLAG%"=="wscv" set CERTIFICATE=/s my /i "Windows Phone Intermediate 2013" /n "Windows Phone OEM Test Cert 2013 (TEST ONLY)" /fd SHA256
if "%CERTIFICATEFLAG%"=="hal" set CERTIFICATE=/s my /i "Windows Phone Intermediate 2013" /n "Windows Phone OEM HAL Extension Test Cert 2013 (TEST ONLY)" /fd SHA256
...
if "%CERTIFICATEFLAG%"=="" set SCENARIO_NAME=CSWP2FileHash

REM Convert from phone scenario name to windows scenario name
if "%CERTIFICATEFLAG%"=="wscv" set SCENARIO_NAME=CSWP2FileHash
...
REM First pass conversion gave this but based on conversation with Nazmus we can just use UAP scenarios everywhere.
REM if "%CERTIFICATEFLAG%"=="msapp" set SCENARIO_NAME=XapStorewpbmsSha2
REM if "%CERTIFICATEFLAG%"=="app2" set SCENARIO_NAME=XapStorewpboemSha2
REM if "%CERTIFICATEFLAG%"=="app3" set SCENARIO_NAME=XapStorewpbmktplaceSha2
REM if "%CERTIFICATEFLAG%"=="msappx" set SCENARIO_NAME=AppStorewpbmsSha2
REM if "%CERTIFICATEFLAG%"=="appx2" set SCENARIO_NAME=AppStorewpboemSha2
REM if "%CERTIFICATEFLAG%"=="appx3" set SCENARIO_NAME=AppStorewpbmktplaceSha2
...
set SIGNER_ARG=authenticode

if "%ENABLE_PRS_SIGN_DEVICE_PRODUCTION%"=="1" (
    REM Production certificates
    set SIGNER_ARG=%SIGNER_ARG% -type ReleaseSignAlways
) ELSE if "%ENABLE_PRS_SIGN_DEVICE_FLIGHT%"=="1" (
    REM Flight certificates
    set SIGNER_ARG=%SIGNER_ARG% -type FlightDigestSignAlways
    ...
    REM Test signing
    set SIGNER_ARG=%SIGNER_ARG% -type TestSignOnly
)
...
echo %RazzleToolPath%\urtrun.cmd 4.0 signer.exe %SIGNER_ARG% -s %SCENARIO_NAME% %FILELIST%
%RazzleToolPath%\urtrun.cmd 4.0 signer.exe %SIGNER_ARG% -s %SCENARIO_NAME% %FILELIST%
ENDLOCAL
